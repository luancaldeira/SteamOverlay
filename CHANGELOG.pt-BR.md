...
Versão só de documentação. Nenhuma mudança na detecção, na comparação ou no
app em si.

- **Inglês passa a ser o idioma principal** do README e deste changelog. As
  versões em português continuam sincronizadas em `README.pt-BR.md` e
  `CHANGELOG.pt-BR.md`, linkadas no topo de cada arquivo.
- **Plano de suporte a Linux adicionado**:
  [`docs/LINUX-SUPPORT-PLAN.md`](docs/LINUX-SUPPORT-PLAN.md), um plano de
  implementação em 7 fases, tarefa por tarefa — descoberta da Steam,
  detecção de hardware, integração com Proton/ProtonDB, degradação por
  gerenciador de janelas, e empacotamento AppImage/deb com CI. Nada do
  plano está implementado ainda; o app continua só para Windows.
